...
public class CareerServiceImpl implements CareerService {
    ...
    public ResponseEntity<?> findById(Long idCareer) {
        return repository.findById(idCareer).map(value -> ResponseEntity.status(HttpStatus.FOUND).body(value))
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).build());
    }
    ...
    public ResponseEntity<?> findCareersByFaculty(Faculty faculty) {
        return repository.findCareersByFaculty(faculty).map(value -> ResponseEntity.status(HttpStatus.FOUND).body(value))
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).build());
    }
}
